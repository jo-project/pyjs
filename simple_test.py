...
"""
pyjs.eval_js(continue_labels_test)
...
"""
pyjs.eval_js(break_labels_test)

assert pyjs.eval_js('(new Date("2008-9-03T20:56:35.450686Z")).toString()')

assert pyjs.eval_js('/ser/.test("Mleko + ser to nabial")')
assert pyjs.eval_js('1 + "1"') == '11'

assert pyjs.eval_js('function (r) {return r}')(5) == 5

x, c = pyjs.run_file('examples/esprima.js')
...
assert pyjs.eval_js('var x = {get y() { return this.val;}, val: 11};x.y') == 11
try:
    assert pyjs.eval_js('syntax error!') and 0
except pyjs.PyJsException as err:
    assert str(err).startswith('SyntaxError: ')


assert pyjs.eval_js('pyimport time; time.time()') <= time.time()

pyjs.disable_pyimport()
try:
    assert pyjs.eval_js('pyimport time') and 0
except pyjs.PyJsException as err:
    assert str(err).startswith('SyntaxError: ')
...
CryptoJS = pyjs.require('crypto-js@3.1.8')
data = [{'id': 1}, {'id': 2}]
...
AES = pyjs.require('crypto-js/aes@3.1.8')
...
esprima = pyjs.require('esprima')
escodegen = pyjs.require('escodegen')
...
#sample_js_code = pyjs.get_file_contents('examples/esprima.js')
...
# chalk = pyjs.require('chalk', include_polyfill=True)
